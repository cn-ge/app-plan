	...
	httpOptions = {
  	...
  	addCours(planning: Planning, cours: Cours, libelleModule: string) {
		...
		coursplanning = {
  		...
  		return this.http.post<CoursPlanning>(API.cours_planning, coursplanning, this.httpOptions);
	}
  	...
  	deleteCours(cours: CoursPlanning) {
  		return this.http.delete<CoursPlanning>(API.cours_planning + '/' + cours.id);
	}
	  
	getCours(idPlanning: number) {
		return this.http.get(API.cours_planning + '/' + idPlanning);
	}
